...
public class uvManager extends uvPlayer {
    ...
    private List<String> targetUsers = new ArrayList<String>(); // The names of the poorly stalked users
    
    public uvManager (Player base) {
        super(base);
        enabled = false;
        if (this.playerInstance != null) playerInstance.isManager = true;
    }   
    ...
    public void resetAsManager () {
        if (enabled) {
            targetUsers.clear();
            enabled = false;
            if (this.playerInstance != null) playerInstance.isManager = false;
            playerInstance = null;
        }
    }     
    ...
    public boolean addVisionTarget (String targetUser) {        
        if (this.enabled) {
            if (targetUsers == null) targetUsers = new ArrayList<String>();
